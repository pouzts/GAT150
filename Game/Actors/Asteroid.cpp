...
void Asteroid::OnCollision(Actor* actor)
{
	if (dynamic_cast<Projectile*>(actor) && actor->tag == "Player")
	{
		...
		std::shared_ptr<PhoenixEngine::Texture> asteroidParticle = scene->engine->Get<PhoenixEngine::ResourceSystem>()->Get<PhoenixEngine::Texture>("sprites/asteroidpart.png", scene->engine->Get<PhoenixEngine::Renderer>());
		scene->engine->Get<PhoenixEngine::ParticleSystem>()->Create(transform.position, 10, 2, asteroidParticle, 50, transform.rotation, PhoenixEngine::DegToRad(30));
